...
def motor(cap):
    # the move_servo function will send the instruction to our pyfirmata library which will send it to our arduino
    def move_servo(angle):
        pin9.write(angle)

    # Since we cannot create a dotted line in openCV directly, I wrote a function that will take two points and create a dotted line between them.
    ...
    # distance is a variable that we will use later in our code. But we must initiate it before our while loop. so I am providing it with a garbage value
    distance = -19723086135
    with mp_hands.Hands(
            min_detection_confidence=0.5,
            min_tracking_confidence=0.5) as hands:
        cv2.namedWindow('image')  # Tambahkan baris ini untuk membuat jendela 'image'
        while cap.isOpened():
            ...
            # The next line is used to create a rectangle with x,y and w,h coordinates
            ...
            if results.multi_hand_landmarks:
                # for each hand, let us get its finger_no , x coordinate, y coordinate as a list
                ...
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    # Gerakkan motor servo berdasarkan posisi x dari tangan
                    hand_x = lmList[9][1]  # Menggunakan titik tengah tangan (id 9)
                    servo_angle = np.interp(hand_x, [0, w], [0, 180])  # Interpolasi posisi x ke sudut servo
